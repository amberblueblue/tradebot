...
当前阶段不需要填写 Binance API key，也不会读取私钥或账户信息。
...
- 不实现真实撤单接口
- 不实现 live broker
- 不允许通过 Dashboard 切换 live
- `run_bot.py` 仍只允许 `paper` 模式；`app.mode=live` 会被 live gate 拦截

即使配置了 Binance API key，当前项目也只会执行只读账户查询，不会发送交易请求。
